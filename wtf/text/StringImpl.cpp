...
PassRefPtr<StringImpl> StringImpl::fill(UChar character, LastCharacterBehavior behavior)
{
    if (!m_length)
        return this;

    bool shouldObscureLastCharacter = (behavior == ObscureLastCharacter);
    if (!(character & ~0x7F) && (!shouldObscureLastCharacter || is8Bit())) {
        LChar* data;
        RefPtr<StringImpl> newImpl = createUninitialized(m_length, data);
        for (unsigned i = 0; i < m_length; ++i)
            data[i] = character;
        unsigned lastCharacterIndex = m_length - 1;
        data[lastCharacterIndex] = shouldObscureLastCharacter ? character : m_data8[lastCharacterIndex];
        return newImpl.release();
    }
    UChar* data;
    RefPtr<StringImpl> newImpl = createUninitialized(m_length, data);
    for (unsigned i = 0; i < m_length; ++i)
        data[i] = character;
    if (!shouldObscureLastCharacter) {
        unsigned lastCharacterIndex = m_length - 1;
        data[lastCharacterIndex] = is8Bit() ? m_data8[lastCharacterIndex] : m_data16[lastCharacterIndex];
    }
    return newImpl.release();
}
